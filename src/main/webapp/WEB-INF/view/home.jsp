...
<body>
	<c:if test="${empty sessionScope.email && empty sessionScope.adminId}">
	<h1>로그인</h1>
	...
	<c:choose>
		<c:when test="${not empty sessionScope.adminId}">
			<div>${sessionScope.adminId}님 반갑습니다.</div>
	...
	<c:choose>
		<c:when test="${not empty sessionScope.email}">
			<div><a href="${pageContext.request.contextPath}/memberOne">${sessionScope.email}님 반갑습니다.</a></div>
        ...
        <tr>
            ...
            <td>
                <c:choose>
                    ...
                    <c:otherwise>
                        <a href="${pageContext.request.contextPath}/notice/noticeOne.jsp?noticeNo=${notice.noticeNo}">
                            ${notice.noticeTitle}
                        </a>
                    </c:otherwise>
                </c:choose>
                <!-- 0 이상일때 댓글 수 표시 -->
                <c:if test="${commentCounts[notice.noticeNo] > 0}">
                    <span>(${commentCounts[notice.noticeNo]})</span>
                </c:if>
            </td>
        </tr>
    </c:forEach>
